// Specialized final Finished Picture apply shader.
// Compiled three times (SDR, scRGB, PQ) so each PSO contains only the colour-space math it can execute.
// The equations are intentionally identical to modes 2/3/4 in dlssnr_finished_color.hlsl.
...
void CSMain(uint3 id : SV_DispatchThreadID)
{
...
#if defined(FINISHED_APPLY_SDR)
    float3 light = pow(max(pixel.rgb, 0.0), 2.2);
    light *= gain;
    float3 result = pow(saturate(light), 1.0 / 2.2);
#elif defined(FINISHED_APPLY_SCRGB)
    float3 result = pixel.rgb * gain;
#elif defined(FINISHED_APPLY_PQ)
    ...
    float3 light = mul(to709, DecodePQ(pixel.rgb));
    light *= gain;
    float3 result = EncodePQ(mul(to2020, light));
#else
#error Select exactly one FINISHED_APPLY_* variant.
#endif
